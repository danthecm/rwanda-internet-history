import { LANDING_POINT_IDS, NODE_COPY } from "~/data/routing.js";
...
function buildRinex(peering) {
  const { exchange, peerCount, routeServerPeers, capacityMbps, facility } = peering;
  ...
  return {
    ...
    status: facility.status === "Active" ? "Active · Operational" : facility.status,
    telemetry: [
      { label: "Connected peers", value: `${peerCount} (${routeServerPeers} via RS)` },
...
function buildStation(id, subsea) {
  ...
  return {
    fields: [
      ...
      {
        label: "Coordinates",
        value: station.coordinates ? formatCoordinates(station.coordinates) : "—",
      },
    ],
 ...
 * @param {{ peering: object | null, subsea: object | null }} data
 */
export function buildInspectorNode(id, { peering, subsea }) {
  ...
  return { name: copy.name, accent: copy.accent, description: copy.description, ...detail };
}
